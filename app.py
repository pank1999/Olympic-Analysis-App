...
st.sidebar.title("Olympic Analysis System")
...
if user_menu=='Medal Tally':
    ...
    if selected_year!='Overall' and selected_country=='Overall':
        st.title("Medal Tally in Year "+str(selected_year)+" Olympic")
    if selected_year!='Overall' and selected_country!='Overall':
        st.title(selected_country+" performance in Year "+str(selected_year))
...
if user_menu=='Country-wise-analysis':
    ...
    country_df=helper.yearWiseMedalTally(df,selected_country)
    fig=px.line(country_df,x="Year", y="Medal")
    st.title(selected_country+" Medal Tally Over The Year") 
    st.plotly_chart(fig)

    st.title(selected_country+" excels in the follwing sports")
    ...
    top10_df=helper.most_successfull_country_wise(df,selected_country)
    st.table(top10_df)
...
if user_menu=='Athlete-wise-analysis':
    ...
    st.title("Men vs Women Participations Over Years")
    ...
    famous_sports=['Basketball','Judo','Football','Tug-Of-War','Athletics','Swimming','Badminton','Sailing','Gymnastics',
                  'Art Competition','Handball','Weightlifting','Wrestling','Water Polo','Hockey','Rowing','Shooting','Boxing',
                  'Cycling','Diving','Table Tennis','Baseball','Golf','Tennis','Polo','Ice Hockey']
    ...
    st.title("Distributions of Age SportsWise(Gold Medalist)")
    fig.update_layout(autosize=False,width=800,height=500)
    st.plotly_chart(fig)
    '''